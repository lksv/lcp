    ...
    class RansackApplicator
      ...
      def define_ransackable_associations
        ...
        @model_class.define_singleton_method(:ransackable_associations) do |auth_object = nil|
          if auth_object.is_a?(LcpRuby::Authorization::PermissionEvaluator)
            # When Ransack traverses nested associations (e.g., product -> category -> parent),
            # it passes the root model's evaluator to each intermediate model. Only apply FK
            # readability checks when the evaluator belongs to this model; for cross-model
            # traversal, allow all associations so nested filters work correctly.
            if auth_object.model_name == model_name
              lcp_assocs
