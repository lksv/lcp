module LcpRuby
  module Metadata
    class ViewGroupDefinition
      ...
      attr_reader :name, :model, :primary_page, :navigation_config, :views, :breadcrumb_config, :public, :raw_hash
      ...
      def presenter_definitions
        page_names.filter_map do |name|
          page = LcpRuby.loader.page_definitions[name]
          next unless page
          LcpRuby.loader.presenter_definitions[page.main_presenter_name]
        end
      end
      ...
      def validate!
        ...
        raise MetadataError, "View group '#{@name}' requires a primary page" if @primary_page.blank?

        unless page_names.include?(@primary_page)
          raise MetadataError,
            "View group '#{@name}': primary page '#{@primary_page}' is not in the views list"
        end
      end
    end
  end
