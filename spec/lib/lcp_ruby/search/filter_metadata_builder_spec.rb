...
RSpec.describe LcpRuby::Search::FilterMetadataBuilder do
  ...
  describe "recursive association traversal (has_many)" do
    let(:tag_model_hash) do
      {
        "name" => "tag",
        "label" => "Tag",
        "fields" => [
        ...
        "associations" => [
          { "type" => "belongs_to", "name" => "product", "target_model" => "product", "foreign_key" => "product_id", "required" => false }
        ],
        "options" => { "label_method" => "name" }
      }
    end

    let(:product_with_has_many_hash) do
      hash = product_model_hash.dup
      hash["associations"] = (hash["associations"] || []) + [
        { "type" => "has_many", "name" => "tags", "target_model" => "tag" }
      ]
      hash
    end

    let(:model_def_with_hm) { LcpRuby::Metadata::ModelDefinition.from_hash(product_with_has_many_hash) }
    let(:tag_model_definition) { LcpRuby::Metadata::ModelDefinition.from_hash(tag_model_hash) }
    ...
    before do
      allow(LcpRuby.loader).to receive(:model_definition).with("tag").and_return(tag_model_definition)
      allow(LcpRuby.loader).to receive(:model_definition).with("product").and_return(model_def_with_hm)
    end

    it "includes has_many association fields in auto-detect" do
      builder = described_class.new(presenter_definition, model_def_with_hm, admin_evaluator)
      result = builder.build

      field_names = result[:fields].map { |f| f[:name] }
      expect(field_names).to include("tags.name", "tags.color")
    end

    it "sets group label for has_many association fields" do
      builder = described_class.new(presenter_definition, model_def_with_hm, admin_evaluator)
      result = builder.build

      tag_field = result[:fields].find { |f| f[:name] == "tags.name" }
      expect(tag_field[:group]).to eq("Tags")
    end

    it "excludes inverse FK field (product_id on tags)" do
      builder = described_class.new(presenter_definition, model_def_with_hm, admin_evaluator)
      result = builder.build

      field_names = result[:fields].map { |f| f[:name] }
      expect(field_names).not_to include("tags.product_id")
    end
  end

  describe "filterable_fields_except" do
    let(:presenter_hash) do
      {
        "name" => "product",
        "model" => "product",
        ...
        "search" => {
          ...
          "advanced_filter" => {
            "enabled" => true,
            "filterable_fields_except" => %w[description sku]
          }
        }
      }
    end
    ...
    it "excludes specified fields from auto-detect" do
      builder = described_class.new(presenter_definition, model_definition, admin_evaluator)
      result = builder.build

      field_names = result[:fields].map { |f| f[:name] }
      expect(field_names).to include("name", "price", "status")
      expect(field_names).not_to include("description", "sku")
    end

    it "excludes entire association subtree" do
      # Exclude the "category" association entirely
      excl_presenter = LcpRuby::Metadata::PresenterDefinition.from_hash(
        "name" => "product",
        "model" => "product",
        "slug" => "products",
        "search" => {
          ...
          "advanced_filter" => {
            "enabled" => true,
            "filterable_fields_except" => %w[category]
          }
        }
      )

      builder = described_class.new(excl_presenter, model_definition, admin_evaluator)
      result = builder.build

      field_names = result[:fields].map { |f| f[:name] }
      expect(field_names).not_to include("category.name")
      expect(field_names).to include("name") # direct fields are still there
    end
  end

  describe "cycle prevention" do
    let(:self_ref_model_hash) do
      {
        "name" => "node",
        "label" => "Node",
        "fields" => [
        ...
        "associations" => [
          { "type" => "belongs_to", "name" => "parent", "target_model" => "node", "foreign_key" => "parent_id", "required" => false },
          { "type" => "has_many", "name" => "children", "target_model" => "node" }
        ],
        "options" => { "label_method" => "name" }
      }
    end

    let(:node_model_def) { LcpRuby::Metadata::ModelDefinition.from_hash(self_ref_model_hash) }

    let(:node_perm_hash) do
      {
        "model" => "node",
        "roles" => {
          "admin" => {
    ...
    let(:node_perm_def) { LcpRuby::Metadata::PermissionDefinition.from_hash(node_perm_hash) }
    let(:node_evaluator) { LcpRuby::Authorization::PermissionEvaluator.new(node_perm_def, admin_user, "node") }

    let(:node_presenter_hash) do
      {
        "name" => "node",
        "model" => "node",
        "slug" => "nodes",
    ...
    let(:node_presenter) { LcpRuby::Metadata::PresenterDefinition.from_hash(node_presenter_hash) }

    before do
      allow(LcpRuby.loader).to receive(:model_definition).with("node").and_return(node_model_def)
    end

    it "does not infinite loop on self-referential models" do
      builder = described_class.new(node_presenter, node_model_def, node_evaluator)
      # Should complete without hanging
      result = builder.build

      field_names = result[:fields].map { |f| f[:name] }
      expect(field_names).to include("name")
      # May include "parent.name" (depth 1) but should NOT recurse infinitely
      expect(field_names.count { |n| n.start_with?("parent.") || n.start_with?("children.") }).to be <= 4
    end
  end

  describe "max_nesting_depth in config output" do
    let(:presenter_hash) do
      {
        "name" => "product",
        "model" => "product",
    ...
    it "passes max_nesting_depth through in config" do
      builder = described_class.new(presenter_definition, model_definition, admin_evaluator)
      result = builder.build

      expect(result[:config][:max_nesting_depth]).to eq(3)
    end
  end
end
