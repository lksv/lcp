...
**Status:** In Progress (Phase 0–4 implemented, Phase 5 deferred)
**Date:** 2026-03-02
...
presenter:
  ...
  search:
    ...
    advanced_filter:
      ...
      max_conditions: 20               # Safety limit on filter rows (default: 10)
      max_association_depth: 3          # How deep association traversal goes (default: 1)
...
model_class.define_singleton_method(:ransackable_scopes) do |_auth_object = nil|
  ...
  # in apply_advanced_search (step 2), not via Ransack's scope mechanism
  []
end
```
...
module LcpRuby
  module Search
    class CustomFieldFilter
      ...
      def self.apply(scope, table_name, field_name, operator, value, cast: nil)
        # Validates field name, extracts JSON path, applies optional type cast,
        # and builds a SQL WHERE condition using Arel.sql.
        # Supports all OperatorRegistry operators: eq, not_eq, cont, not_cont,
        # start, not_start, end, not_end, gt, gteq, lt, lteq, between, in,
        # not_in, present, blank, null, not_null, true, false, not_true, not_false.
        # DB-portable: uses ILIKE on PostgreSQL, LIKE with ESCAPE on SQLite/MySQL.
        # ...
      end

      # Parse composite cf key "field_name_operator" into [field_name, operator].
      def self.parse_cf_key(key, sorted_field_names)
        # Longest-match strategy: sorted_field_names is sorted by length desc.
        # ...
      end

      # Map custom field type to SQL cast (:integer, :decimal, :date).
      def self.cast_for_type(custom_type)
        # ...
      end
    end
  end
...
| `is not true` | `_not_true` | Not boolean true |
| `is false` | `_false` | Boolean false |
| `is not false` | `_not_false` | Not boolean false |
| `is this_week` | expanded to `_gteq` + `_lteq` | This week |
| `is this_month` | expanded to `_gteq` + `_lteq` | This month |
| `is this_quarter` | expanded to `_gteq` + `_lteq` | This quarter |
| `is this_year` | expanded to `_gteq` + `_lteq` | This year |
...
**`filter_data` structure** (JSON, mirrors the visual builder state — recursive tree with `combinator` + `children`):
...
Each child is either a leaf condition (`{ "field", "operator", "value" }`) or a nested group (`{ "combinator", "children" }`). This recursive tree replaces the earlier two-level `{ conditions, groups }` format. The `FilterParamBuilder` accepts both formats for backward compatibility.
...
```json
{
  "fields": [
    ...
    { "name": "stage", "label": "Stage", "type": "enum", "operators": ["eq","not_eq","in","not_in"], "enum_values": [["lead","Lead"],["qualified","Qualified"]] },
    { "name": "company.name", "label": "Name", "type": "string", "group": "Company", "operators": ["eq","cont","start"] }
  ],
  "operator_labels": { "eq": "equals", "cont": "contains", "..." : "..." },
  "no_value_operators": ["present", "blank", "null", "not_null", "true", "not_true", "false", "not_false"],
...
The JavaScript builds a field tree from the flat metadata for the cascading field picker, detects the active preset by comparing URL-parsed state, and renders preset buttons with active highlighting. No-value operators (`true`, `present`, `this_month`, etc.) are encoded with value `"1"` in URL params so Ransack activates the predicate.
...
**Phase rationale (all phases 0–4 are now implemented):**
...
20b. Implement cascading field picker — replace flat `<select>` with cascading drill-down selects for association fields (client-side JS, tree built from flat dot-path metadata)
20c. Implement recursive condition nesting — full AND/OR tree with configurable `max_nesting_depth`
20d. Implement filter presets UI — render preset buttons from `advanced_filter.presets` metadata, one-click apply with active state highlighting, automatic URL round-trip matching

### Phase 3: Custom Field Filtering (**Implemented**)
...
### Phase 4: Query Language (**Implemented**)
...
31. ~~Implement YAML presets loading~~ — **Done.** Presets are defined in `advanced_filter.presets`, passed through `FilterMetadataBuilder`, and rendered as one-click pill buttons in the filter UI with active state highlighting.
