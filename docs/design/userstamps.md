...
### Integration with Model Options Infrastructure

Userstamps follows the patterns defined in [Model Options Infrastructure](model_options_infrastructure.md):

- **ModelDefinition**: uses the shared `boolean_or_hash_option("userstamps")` helper (§3)
- **ConfigurationValidator**: uses `validate_boolean_or_hash_option` for type/key validation (§4)
- **Builder pipeline**: runs as `apply_userstamps` after `apply_positioning` and after `apply_auditing` (§1) — userstamp fields (`updated_by_id`) appear in `saved_changes`, so the auditing callback captures them correctly
- **`update_columns` bypass contract**: userstamps accepts that `update_columns`/`update_all` bypass the `before_save` callback — this is by design, same rationale as Rails timestamps (§2, §9 general rule #2)
- **Cross-feature interactions**: see infrastructure §9 for soft_delete+userstamps, auditing+userstamps, and positioning+userstamps resolutions
...
Uses the shared `boolean_or_hash_option` helper from [Model Options Infrastructure §3](model_options_infrastructure.md):

```ruby
# lib/lcp_ruby/metadata/model_definition.rb

# Shared helper (see model_options_infrastructure.md §3)
# boolean_or_hash_option("userstamps") → [enabled?, options_hash]
...
Uses the shared `validate_boolean_or_hash_option` helper from [Model Options Infrastructure §4](model_options_infrastructure.md) for type/key validation, then adds feature-specific checks:

```ruby
def validate_userstamps(model)
  # Shared validation: type check + unknown keys (see model_options_infrastructure.md §4)
  opts = validate_boolean_or_hash_option(model, "userstamps",
    allowed_keys: %w[created_by updated_by store_name])
  return unless opts

  # Feature-specific: column names don't clash with defined fields
...
4. `Builder` — call `apply_userstamps` in the canonical pipeline position (after `apply_positioning`, before `apply_external_fields` — see [Model Options Infrastructure §1](model_options_infrastructure.md))
...
- **[Model Options Infrastructure](model_options_infrastructure.md):** Defines the canonical Builder pipeline, shared `boolean_or_hash_option` helper, `update_columns` bypass contract, `LcpRuby::UserSnapshot` shared utility, and cross-feature interaction matrix. Userstamps follows these patterns (see §1, §3, §4, §8, §9).
- **[Auditing](auditing.md):** Auditing tracks the full history of changes with user snapshots. Userstamps are a lightweight alternative that only tracks current creator and last modifier. Both features are complementary — userstamps provide quick "who created/modified this" without querying an audit log. Auditing captures userstamp field changes in diffs by default (see infrastructure §9).
- **[Soft Delete](soft_delete.md):** `discard!` uses `update_columns` which bypasses the userstamps `before_save` callback — `updated_by_id` is NOT updated on discard. This is by design: the audit log captures who discarded (see infrastructure §9).
- **[Multiselect and Batch Actions](multiselect_and_batch_actions.md):** Batch `update` uses per-record `record.update(...)` which fires callbacks, so userstamps work normally. Batch operations using `BulkUpdater.tracked_update_all` bypass userstamps by design.
